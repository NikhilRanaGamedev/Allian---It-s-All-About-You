...
$pic_path = "users/$username/profile_pics/profile.jpg";
...
if($_SERVER["REQUEST_METHOD"]=="POST") {
  if(isset($_POST["btn_upload_profile"])) {
    ...
    // Check file size
    if ($_FILES["dp_btn"]["size"] > 10000000) {
    ...
    if ($uploadOk == 0) {
      ...
      if (move_uploaded_file($_FILES["dp_btn"]["tmp_name"], "users/".$_SESSION['username']."\profile_pics/profile.jpg")) {
       echo "<script>alert('The file ". basename( $_FILES["dp_btn"]["name"]). " has been uploaded.')</script>";
       $value = mysqli_query($conn, "UPDATE `user_info` SET `profile_pic` = '$pic_path' where user_id = $user_id");
